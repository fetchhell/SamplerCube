...
Camera::Camera() {
	...
	NvBuildIdentityMatf(m_model);
	NvBuildRotXRadMatf(m_model, -3.14/2.0);

	NvBuildIdentityMatf(m_view);
	NvBuildIdentityMatf(m_projection);

	NvBuildIdentityMatf(m_modelView);
	NvBuildIdentityMatf(m_viewProjection);

	NvBuildIdentityMatf(m_modelViewProjection);
...
//-------------------------------------------------------
void Camera::buildModelViewProjection() {

	/* building modelView & viewProjection matrices */
	NvMultMatf(m_modelView, m_view, m_model);
	NvMultMatf(m_viewProjection, m_projection, m_view);

	/* building modelViewProjection matrix */
	NvMultMatf(m_modelViewProjection, m_projection, m_modelView);
}